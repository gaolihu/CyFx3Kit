...
bool AppStateMachine::processEvent(StateEvent event, const QString& reason) {
    ...
    LOG_INFO(QString::fromLocal8Bit("处理状态事件: %1, 状态: %2, 原因: %3")
        .arg(eventStr)
        .arg(stateStr)
        .arg(reason));
...
void AppStateMachine::executeStateChange(AppState newState, const QString& reason, bool isError) {
    ...
    LOG_INFO(QString::fromLocal8Bit("执行状态转换: %1 -> %2, 原因: %3")
        .arg(oldStateStr)
        .arg(newStateStr)
        .arg(reason));
    ...
    QMetaObject::invokeMethod(this, [this, oldState, newState, reason]() {
        LOG_WARN(QString::fromLocal8Bit("发出leavingState信号，新状态: %1, 旧状态: %2").arg(stateToString(newState)).arg(stateToString(oldState)));
        emit leavingState(oldState, reason);

        LOG_WARN(QString::fromLocal8Bit("发出stateChanged信号，新状态: %1, 旧状态: %2").arg(stateToString(newState)).arg(stateToString(oldState)));
        emit stateChanged(newState, oldState, reason);

        LOG_WARN(QString::fromLocal8Bit("发出enteringState信号，新状态: %1, 旧状态: %2").arg(stateToString(newState)).arg(stateToString(oldState)));
        emit enteringState(newState, reason);
        }, Qt::QueuedConnection);

    if (isError) {
        LOG_ERROR(QString::fromLocal8Bit("发出errorOccurred信号，原因: %1").arg(reason));
...
StateTransitionResult AppStateMachine::handleEvent(AppState currentState, StateEvent event, const QString& reason) {
    ...
    if (event == StateEvent::APP_SHUTDOWN) {
        return StateTransitionResult(AppState::SHUTDOWN, QString::fromLocal8Bit("应用程序正在关闭"));
    }
    ...
    case AppState::INITIALIZING:
        ...
        case StateEvent::DEVICE_CONNECTED:
            return StateTransitionResult(AppState::COMMANDS_MISSING, QString::fromLocal8Bit("设备已连接，等待命令文件"));
    ...
    case AppState::DEVICE_ERROR:
        switch (event) {
        case StateEvent::DEVICE_CONNECTED:
            return StateTransitionResult(AppState::COMMANDS_MISSING, QString::fromLocal8Bit("设备已重新连接，等待命令文件"));
        case StateEvent::DEVICE_DISCONNECTED:
            return StateTransitionResult(AppState::DEVICE_ABSENT, QString::fromLocal8Bit("设备已断开连接"));
        default:
            break;
        }
        break;
    ...
    case AppState::COMMANDS_MISSING:
        ...
        case StateEvent::COMMANDS_LOADED:
            return StateTransitionResult(AppState::CONFIGURED, QString::fromLocal8Bit("命令文件已加载，系统已配置"));
        case StateEvent::DEVICE_DISCONNECTED:
            return StateTransitionResult(AppState::DEVICE_ABSENT, QString::fromLocal8Bit("设备已断开连接"));
        case StateEvent::ERROR_OCCURRED:
            return StateTransitionResult(AppState::DEVICE_ERROR, reason, true);
    ...
    case AppState::CONFIGURED:
        ...
        case StateEvent::START_REQUESTED:
            return StateTransitionResult(AppState::STARTING, QString::fromLocal8Bit("正在启动数据传输"));
        case StateEvent::COMMANDS_UNLOADED:
            return StateTransitionResult(AppState::COMMANDS_MISSING, QString::fromLocal8Bit("命令文件已卸载"));
        case StateEvent::DEVICE_DISCONNECTED:
            return StateTransitionResult(AppState::DEVICE_ABSENT, QString::fromLocal8Bit("设备已断开连接"));
        case StateEvent::ERROR_OCCURRED:
            return StateTransitionResult(AppState::DEVICE_ERROR, reason, true);
    ...
    case AppState::STARTING:
        ...
        case StateEvent::START_SUCCEEDED:
            return StateTransitionResult(AppState::TRANSFERRING, QString::fromLocal8Bit("数据传输已开始"));
        case StateEvent::START_FAILED:
            return StateTransitionResult(AppState::DEVICE_ERROR, QString::fromLocal8Bit("启动数据传输失败: ") + reason, true);
        case StateEvent::DEVICE_DISCONNECTED:
            return StateTransitionResult(AppState::DEVICE_ABSENT, QString::fromLocal8Bit("设备已断开连接"));
        case StateEvent::ERROR_OCCURRED:
            return StateTransitionResult(AppState::DEVICE_ERROR, reason, true);
    ...
    case AppState::TRANSFERRING:
        ...
        case StateEvent::STOP_REQUESTED:
            return StateTransitionResult(AppState::STOPPING, QString::fromLocal8Bit("正在停止数据传输"));
        case StateEvent::DEVICE_DISCONNECTED:
            return StateTransitionResult(AppState::DEVICE_ABSENT, QString::fromLocal8Bit("设备已断开连接"));
        case StateEvent::ERROR_OCCURRED:
            return StateTransitionResult(AppState::DEVICE_ERROR, reason, true);
    ...
    case AppState::STOPPING:
        ...
        case StateEvent::STOP_SUCCEEDED:
            return StateTransitionResult(AppState::CONFIGURED, QString::fromLocal8Bit("数据传输已停止"));
        case StateEvent::STOP_FAILED:
            return StateTransitionResult(AppState::DEVICE_ERROR, QString::fromLocal8Bit("停止数据传输失败: ") + reason, true);
        case StateEvent::DEVICE_DISCONNECTED:
            return StateTransitionResult(AppState::DEVICE_ABSENT, QString::fromLocal8Bit("设备已断开连接"));
        case StateEvent::ERROR_OCCURRED:
            return StateTransitionResult(AppState::DEVICE_ERROR, reason, true);
    ...
    default:
        LOG_ERROR(QString::fromLocal8Bit("未知状态: %1").arg(static_cast<int>(currentState)));
        break;
    }
...
QString AppStateMachine::stateToString(AppState state) {
    switch (state) {
    case AppState::INITIALIZING: return QString::fromLocal8Bit("初始化中");
    case AppState::DEVICE_ABSENT: return QString::fromLocal8Bit("设备未连接");
    case AppState::DEVICE_ERROR: return QString::fromLocal8Bit("设备错误");
    case AppState::IDLE: return QString::fromLocal8Bit("空闲");
    case AppState::COMMANDS_MISSING: return QString::fromLocal8Bit("命令未加载");
    case AppState::CONFIGURED: return QString::fromLocal8Bit("已配置");
    case AppState::STARTING: return QString::fromLocal8Bit("启动中");
    case AppState::TRANSFERRING: return QString::fromLocal8Bit("传输中");
    case AppState::STOPPING: return QString::fromLocal8Bit("停止中");
    case AppState::SHUTDOWN: return QString::fromLocal8Bit("关闭中");
    default: return QString::fromLocal8Bit("未知状态");
    }
}
...
QString AppStateMachine::eventToString(StateEvent event) {
    ...
    default: return QString::fromLocal8Bit("未知事件");
    }
}