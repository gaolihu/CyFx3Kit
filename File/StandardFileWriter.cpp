...
bool StandardFileWriter::open(const QString& filename) {
    ...
    if (!m_isOpen) {
        ...
        LOG_ERROR(fromLocal8Bit("打开文件失败: %1 - %2").arg(filename).arg(m_lastError));
        return false;
    }

    LOG_INFO(fromLocal8Bit("文件已打开: %1").arg(filename));
    return true;
}
...
bool StandardFileWriter::write(const QByteArray& data) {
    ...
    if (written != data.size()) {
        m_lastError = m_file.errorString();
        LOG_ERROR(fromLocal8Bit("文件写入错误: %1").arg(m_lastError));
        return false;
    }
